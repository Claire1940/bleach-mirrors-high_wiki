...
export async function GET() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://bleach-mirrors-high.wiki'
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'BLEACH Mirrors High Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} is a comprehensive anime and manga wiki providing character guides, arc breakdowns, Zanpakuto references, and community resources.`)
  lines.push('')
