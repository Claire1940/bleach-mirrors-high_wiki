...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  setRequestLocale(locale)
  const contentType = slug[0]
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://bleach-mirrors-high.wiki'
  ...
  if (isListPage) {
    ...
    try {
      ...
      // 如果翻译不存在，使用默认值
      const defaultTitle = `${contentType.charAt(0).toUpperCase() + contentType.slice(1)} - Bleach Wiki`
      const path = `/${contentType}`

      return {
        title: defaultTitle,
        description: `Browse all ${contentType} content for Bleach Wiki`,
        alternates: buildLanguageAlternates(path, locale as Locale, siteUrl),
        robots: {
          index: true,
          follow: true,
    ...
    try {
      ...
      return {
        title: `${metadata.title} - Bleach Wiki`,
